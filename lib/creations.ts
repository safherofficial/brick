...
function shade(hex: string, amount: number): string {
  ...
  const mix = (channel: number) =>
    Math.round(Math.min(255, Math.max(0, channel + amount * 255)))
      .toString(16)
      .padStart(2, "0");
...
function worldOrbColor(nx: number, ny: number, nz: number) {
  ...
  const lift = Math.min(
    1,
    spec * 1.4 + Math.max(meridian, equator) * 0.85 + rim * 0.12
  );
...
function worldEmblem(): ShowcaseBrick[] {
  const bricks: ShowcaseBrick[] = [];
  const unit = 0.42;
  const radius = 5;
  const cx = -3.2;
      ...
      for (let z = -radius; z <= radius; z++) {
        ...
        bricks.push(
          b(
            ...
            [cx + x * unit, cy + y * unit, z * unit],
            worldOrbColor(x / radius, y / radius, z / radius),
            { studless: true }
          )
        );
      }
...
function tower(
  ...
  for (let y = 0; y < height; y += BRICK_H) {
    bricks.push(b([1.8, BRICK_H, 1.8], [x, y + BRICK_H / 2, z], body));
    if (y >= 0.9 && Math.round(y / BRICK_H) % 3 === 0) {
      bricks.push(
        b([0.32, 0.24, 0.08], [x, y + 0.08, z + 0.94], accent, {
          studless: true
        })
      );
    }
  }
  bricks.push(b([2.15, 0.3, 2.15], [x, height + 0.15, z], accent));
  bricks.push(
    b([2.0, 0.9, 2.0], [x, height + 0.75, z], roof, { shape: "cone" })
  );
  return bricks;
}
...
function cyberpunkCity(): ShowcaseBrick[] {
  ...
  bricks.push(...tileArea(12.6, 10.8, 0, 0, 0.225, 0.45, road, { seed: 1 }));
  bricks.push(b([10.8, 0.12, 0.9], [0, 0.52, 0], "#111827", { studless: true }));
  bricks.push(b([0.9, 0.12, 9.8], [0, 0.54, 0], "#111827", { studless: true }));
  ...
  buildings.forEach(([x, z, h, w, d], index) => {
    ...
    for (let i = 0; i < levels; i++) {
      bricks.push(
        ...tileArea(w, d, x, z, 0.45 + i * BRICK_H, BRICK_H, index % 2 ? dark2 : dark, {
          seed: index * 97 + i
        })
      );
      if (i > 0 && i % 2 === 0) {
    ...
    bricks.push(
      b([w + 0.18, 0.18, d + 0.18], [x, h + 0.54, z], index % 2 ? cyan : magenta, {
        studless: true
      })
    );
    if (index === 3 || index === 7) {
      bricks.push(b([0.18, 1.2, 0.18], [x, h + 1.2, z], cyan, { studless: true }));
      bricks.push(b([0.7, 0.12, 0.12], [x, h + 1.8, z], magenta, { studless: true }));
    }
  });

  for (let x = -5.2; x <= 5.2; x += 1.3) {
    bricks.push(b([0.12, 0.12, 8.8], [x, 0.62, 0], cyan, { studless: true }));
  }
  for (let z = -4.1; z <= 4.1; z += 1.3) {
    bricks.push(b([10.8, 0.12, 0.12], [0, 0.63, z], magenta, { studless: true }));
  }
  bricks.push(b([5.0, 0.16, 0.18], [0, 4.2, 0], cyan, { studless: true }));
  bricks.push(b([0.18, 0.16, 5.0], [0, 4.25, 0], magenta, { studless: true }));
  return bricks;
}
...
function spaceStation(): ShowcaseBrick[] {
  ...
  bricks.push(...tileArea(4.8, 4.8, 0, 0, 0.225, 0.45, grey, { seed: 2 }));

  let hubLevel = 0;
  for (let y = 0; y < 3.6; y += BRICK_H) {
    bricks.push(
      ...tileArea(2.7, 2.7, 0, 0, 0.45 + y, BRICK_H, white, {
        seed: 200 + hubLevel
      })
    );
    hubLevel += 1;
  ...
  bricks.push(b([3.0, 0.5, 3.0], [0, 4.15, 0], blue, { shape: "cylinder" }));
  bricks.push(b([2.5, 1.2, 2.5], [0, 5.0, 0], white, { shape: "cone" }));
  ...
  wings.forEach(([x, , z], index) => {
    const horizontal = x !== 0;
    for (let i = -2; i <= 2; i++) {
      const p: [number, number, number] = horizontal
        ? [x, 0.9, i * 0.75]
        : [i * 0.75, 0.9, z];
      bricks.push(b(horizontal ? [2.8, 0.3, 0.6] : [0.6, 0.3, 2.8], p, white));
    }
    const panelX = horizontal ? x + (x > 0 ? 1.55 : -1.55) : 0;
    const panelZ = horizontal ? 0 : z + (z > 0 ? 1.55 : -1.55);
    bricks.push(
      b(
        horizontal ? [2.4, 0.12, 3.8] : [3.8, 0.12, 2.4],
        [panelX, 1.08, panelZ],
        blue,
        { studless: true }
      )
    ...
    bricks.push(
      b(
        horizontal ? [0.12, 0.14, 4.0] : [4.0, 0.14, 0.12],
        [panelX, 1.16, panelZ],
        orange,
        { studless: true }
      )
  ...
  for (let i = 0; i < 8; i++) {
    ...
    bricks.push(
      b(
        [0.45, 0.9, 0.45],
        [Math.cos(angle) * 3.1, 1.0, Math.sin(angle) * 3.1],
        orange,
        { shape: "cylinder" }
      )
    );
...
function japaneseCastle(): ShowcaseBrick[] {
  ...
  bricks.push(...tileArea(11, 9, 0, 0, 0.225, 0.45, stone, { seed: 3 }));
  ...
  levels.forEach((level, li) => {
    ...
    bricks.push(
      b([level.w + 0.25, 0.22, level.d + 0.25], [0, level.y + level.h + 0.11, 0], roof)
    );
    bricks.push(
      b([level.w * 0.72, 0.32, level.d * 0.72], [0, level.y + level.h + 0.38, 0], roof2, {
        shape: "cone"
      })
    );
  });

  bricks.push(...tower(-4.3, -3.3, 2.25, stone, stone2, roof));
  bricks.push(...tower(4.3, -3.3, 2.25, stone, stone2, roof));
  bricks.push(...tower(-4.3, 3.3, 2.25, stone, stone2, roof));
  bricks.push(...tower(4.3, 3.3, 2.25, stone, stone2, roof));

  bricks.push(b([1.2, 1.8, 0.25], [0, 1.0, 3.25], wood, { studless: true }));
  bricks.push(b([1.5, 0.16, 0.12], [0, 1.9, 3.38], gold, { studless: true }));

  for (const x of [-2.4, -1.2, 1.2, 2.4]) {
    bricks.push(b([0.45, 0.55, 0.08], [x, 3.15, 2.54], wood, { studless: true }));
    bricks.push(b([0.45, 0.55, 0.08], [x, 4.4, 1.94], wood, { studless: true }));
  }

  bricks.push(b([0.16, 2.4, 0.16], [0, 6.6, 0], gold, { studless: true }));
  bricks.push(b([0.5, 0.25, 0.5], [0, 7.85, 0], gold, { shape: "cone", studless: true }));
  return bricks;
}
...
function airship(): ShowcaseBrick[] {
  ...
  bricks.push(...tileArea(7.2, 3.0, 0, 0, 2.0, 0.45, dark, { seed: 4 }));

  for (let x = -3; x <= 3; x += 1) {
    bricks.push(b([0.75, 0.55, 3.4], [x, 2.45, 0], bronze));
  }

  bricks.push(b([8.0, 0.3, 2.7], [0, 2.85, 0], brass, { studless: true }));
  bricks.push(b([5.8, 0.5, 2.2], [0, 3.25, 0], cream, { shape: "cone", studless: true }));

  for (const x of [-2.8, -1.4, 0, 1.4, 2.8]) {
    bricks.push(b([0.25, 2.1, 0.25], [x, 0.85, 0], bronze, { studless: true }));
    bricks.push(b([0.25, 0.25, 2.8], [x, 0.55, 0], brass, { studless: true }));
  }

  bricks.push(...tileArea(2.8, 1.8, 0, 0, 0.1, 0.4, dark, { seed: 5 }));
  bricks.push(b([1.9, 0.8, 1.5], [0, -0.45, 0], red, { shape: "cone" }));

  for (const x of [-2.7, 2.7]) {
    bricks.push(b([0.45, 1.8, 0.45], [x, 3.8, 0], bronze, { shape: "cylinder" }));
    bricks.push(b([0.8, 0.2, 0.8], [x, 4.75, 0], brass));
    bricks.push(b([0.15, 2.2, 0.15], [x, 5.85, 0], dark, { studless: true }));
  }

  bricks.push(b([0.3, 0.3, 0.3], [0, 5.9, 0], brass, { shape: "cone", studless: true }));
  return bricks;
}
...
function solanaBar(
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < columns; c++) {
      ...
      bricks.push(b([cell, cell, depth], [x, y, 0], solanaGradient(c / (columns - 1))));
    }
  }
  return bricks;
}
...
function solanaEmblem(): ShowcaseBrick[] {
  ...
  for (let barIndex = 0; barIndex < 3; barIndex++) {
    bricks.push(...solanaBar(columns, rows, 0, barIndex * barPitch, shearPerRow, depth));
  }

  bricks.push(
    ...tileArea(
      columns * STUD + shearPerRow * rows + 2.4,
      depth + 2.0,
...
export const creations: ShowcaseCreation[] = [
  {
    ...
    author: "@world",
    description:
      "Voxel sculpture of the World orb: iridescent sphere, specular cross and wordmark.",
    likes: "12.4K",
    views: "68.1K",
    bricks: worldEmblem(),
    camera: [11, 6, 14],
