...
export const STARTER_LIMIT = 100;
...
export const basicKinds: BrickKind[] = ["1x1", "1x2", "2x2", "2x4", "2x6"];
...
export function sizeFor(footprint: Footprint): Vec3 {
  return [footprint[0] * STUD, BRICK_HEIGHT, footprint[1] * STUD];
}
...
export function yFromLayer(layer: number) {
  return layer * BRICK_HEIGHT + BRICK_HEIGHT / 2;
}
...
export function withLayer(brick: Brick, layer: number): Brick {
  return {
    ...brick,
    layer,
    position: [brick.position[0], yFromLayer(layer), brick.position[2]]
  };
}
...
export function settle(bricks: Brick[]) {
  ...
  for (const brick of ordered) {
    const layer = highestSupportedLayer(brick, out);
    out.push(withLayer(brick, layer));
  }
  return out;
}
...
export function makeBrick(
  ...
  const layer = highestSupportedLayer(provisional, bricks);
