...
function modelCandidates(id: AiModelId): string[] {
  ...
  // Client bundle: production builds should not hit third-party hosts on the hot path.
  ...
  try {
    const cores =
      typeof navigator !== "undefined" && navigator.hardwareConcurrency
        ...
        : 2;
    // Cap threads: too many hurts small models on WASM
...
async function createSession(
  ...
  // ort types in 1.21 ship a narrow options shape; runtime accepts these flags.
  const options = {
  ...
  const job = (async () => {
    ...
    try {
      ...
      try {
        ...
        // Self-hosted /ort missing → CDN wasm (dev / first deploy)
...
/**
 * Warm segment session during idle time on /build.
 * Depth is not preloaded (heavier, used only for relief).
 */
