...
async function runMap(
  ...
  /*
   * onnxruntime-web exposes `dims` at runtime on Tensor,
   * but the Tensor type inferred from this session result
   * does not expose it in the current TypeScript definitions.
   *
   * Keep the runtime object untouched and narrow only the
   * missing shape property locally.
   */
  const outputWithDims =
