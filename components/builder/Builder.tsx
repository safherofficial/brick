...
function effectiveFootprint(brick: Brick): [number, number] {
  if (brick.rotation === 90) {
    return [brick.footprint[1], brick.footprint[0]];
  }

  return brick.footprint;
}

function effectiveSize(brick: Brick): [number, number, number] {
  if (brick.rotation === 90) {
    return [
      brick.size[2],
      brick.size[1],
...
function cellsFor(brick: Brick): Array<[number, number]> {
  const [w, d] = effectiveFootprint(brick);
...
function layerOf(brick: Brick) {
  return Math.round(
    (brick.position[1] - BRICK_H / 2) / BRICK_H
  );
}
...
function occupancy(bricks: Brick[]) {
  ...
  for (const brick of bricks) {
    ...
    for (const [x, z] of cellsFor(brick)) {
      const key = cellKey(x, z);

      map.set(
        key,
        Math.max(map.get(key) ?? 0, layer)
      );
    }
  }
...
function placementLayer(
  ...
  const baseFootprint = footprints[kind];

  const fp: [number, number] =
    rotation === 90
      ? [baseFootprint[1], baseFootprint[0]]
      : baseFootprint;
  ...
  for (let ix = 0; ix < fp[0]; ix++) {
    for (let iz = 0; iz < fp[1]; iz++) {
      const key = cellKey(
        Math.round(startX + ix),
        Math.round(startZ + iz)
      );
...
function collides(candidate: Brick, bricks: Brick[]) {
  ...
  const occupied = new Set(
    cellsFor(candidate).map(([x, z]) =>
      cellKey(x, z)
    )
  );

  return bricks.some((brick) => {
    if (layerOf(brick) !== candidateLayer) {
      return false;
    }

    return cellsFor(brick).some(([x, z]) =>
...
function validCandidate(
  ...
  return cellsFor(candidate).every(
    ([x, z]) =>
      (support.get(cellKey(x, z)) ?? 0) === layer
  );
}
...
function Studs({
  ...
  const cols = Math.max(
    1,
    Math.round(size[0] / 0.92)
  );

  const rows = Math.max(
    1,
    Math.round(size[2] / 0.92)
  );
...
function BrickMesh({
  ...
  const renderSize = effectiveSize(brick);
  ...
  const addAt = useCallback(
    (point: THREE.Vector3) => {
      ...
      if (
        ...
        setToast(
          "Position blocked — move to an open cell"
        );
        return;
      }
  ...
  const rotate =
    useCallback(() => {
      ...
      if (
        ...
        setToast(
          "Cannot rotate — occupied cells detected"
        );
      }
    }, [
      bricks,
      ...
      <div className="builderBody">
        <aside className="brickPanel">
          ...
          <div className="partGrid">
            {(
              ...
              >
                <span
                  className={`miniBrick mini-${k.replace(
                    "x",
                    "-"
                  )}`}
                />
        ...
        <section className="scene">
          ...
          <div className="sceneHint">
            CLICK EMPTY GRID TO
            PLACE · CLICK BRICK TO
            SELECT · SHIFT+CLICK TO
            STACK · ARROWS MOVE · R
            ROTATE
          </div>
