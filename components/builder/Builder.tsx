...
export default function Builder() {
  ...
  const [aiCategory, setAiCategory] = useState<AiCategory>("objects");
  ...
  const setStyleForCategory = useCallback((category: AiCategory) => {
    const preset = aiCategoryPreset(category);
  ...
  const imageOptions = useCallback(
    (override?: { symmetrize?: boolean }) =>
      buildImageOptions({
        ...
        heightMax: imageMode === "flat" ? 2 : imageHeight,
        maxVoxels: MAX_SAFE,
    ...
    [aiCategory, imageHeight, imageMode, symmetrize]
  );
  ...
  const activeStyle = aiCategoryPreset(aiCategory).style;
  ...
  const regenerateMultiView = useCallback(
    async ({ front, side }: { front: File; side?: File }) => {
      ...
      setBusy(true);
      try {
        const result = await imagesToVoxels({ front, side }, imageOptions());
        if (job !== imageJobRef.current) return;
        setPendingImage(result);
        const usedViews = Number(Boolean(front)) + Number(Boolean(side));
        notify(usedViews === 2 ? `Preview ready · ${activeStyle} · 2 views · ${result.count} vx` : `Preview ready · ${activeStyle} · ${result.count} vx`);
      } catch (error) {
    ...
    [activeStyle, imageOptions, notify]
  );
  ...
  const attachFront = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const result = sideFile
          ...
          : await imageToVoxels(file, imageOptions({ symmetrize: imageMode === "model" ? symmetrize : false }));
        if (job !== imageJobRef.current) return;
        if (!result.voxels.length) throw new Error("Empty image");
        setPendingImage(result);
        notify(`Preview ready · ${activeStyle} · ${result.count ?? result.voxels.length} vx`);
        setPaywall(false);
      } catch (error) {
    ...
    [activeStyle, imageMode, imageOptions, notify, sideFile, symmetrize]
  );
  ...
  const openProject = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        if (lower.endsWith(".png") || lower.endsWith(".jpg") || lower.endsWith(".jpeg") || lower.endsWith(".webp")) {
          ...
          setFrontFile(file);
          setSideFile(null);
          const result = await imageToVoxels(file, imageOptions({ symmetrize: false }));
          if (job !== imageJobRef.current) return;
          if (!result.voxels.length) throw new Error("Empty image");
          setPendingName(file.name.replace(/\.(png|jpe?g|webp)$/i, ""));
          setPendingHash(await hashImageFile(file));
    ...
    [activeStyle, imageOptions, notify, packVolume]
  );
      ...
      <div className="builderBody voxelBody">
          ...
          <details className="fold">
            ...
            <div className="foldBody">
              <p className="foldHint">
                {tool.toUpperCase()} · {activeStyle.toUpperCase()} · {count} VX · {volume.size}³
          ...
          {pendingImage && !paywall && (
            <div className="toast" style={{ bottom: 24, minWidth: 300 }}>
              <div style={{ marginBottom: 8 }}>
                APPLY IMAGE · {pendingImage.count ?? pendingImage.voxels.length} VX · {activeStyle.toUpperCase()} · {viewLabel}
              </div>
              <div className="viewRow">
        ...
        <aside className="inspector">
          ...
          <details className="fold">
            ...
            <div className="foldBody">
              <p className="foldHint">
                ...
                <br />
                Category selects the reconstruction profile: SWORDS uses weapon, OBJECTS uses prop.
                <br />
          ...
          {(imageMode === "solid" || imageMode === "relief" || imageMode === "model") && (
            <>
              <p className="foldHint">Maximum depth · {activeStyle}</p>
              <div className="viewRow">
                {[4, 8, 12, 16].map((n) => (
                  <button
                    key={n}
          ...
          <p className="category">AI MODE</p>
          {AI_CATEGORY_PRESETS.map((preset) => (
            <button
              key={preset.id}
              className={aiCategory === preset.id ? "modeOn" : ""}
              onClick={() => {
                setStyleForCategory(preset.id);
                notify(`${preset.label} · ${preset.style} · D${preset.heightMax}`);
                if (frontFile) void rebuildMultiView();
              }}
