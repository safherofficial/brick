...
import { BrickVisual, type BrickShape } from "./BrickVisual";
...
function GhostBrick({ position, size, footprint, shape, color, valid }: { position: [number, number, number]; size: [number, number, number]; footprint: [number, number]; shape: BrickShape; color: string; valid: boolean }) {
  return <group position={position}>
    <BrickVisual shape={shape} size={size} footprint={footprint} color={valid ? color : "#ef4444"} opacity={0.3} />
  </group>;
}
...
function BrickMesh({ brick, selected, onSelect, onHover, onPlace }: { brick: Brick; selected: boolean; onSelect: (id: number) => void; onHover: (point: THREE.Vector3) => void; onPlace: (point: THREE.Vector3) => void }) {
  return <group position={brick.position}
    ...
    <BrickVisual shape={brick.shape} size={brick.size} footprint={brick.footprint} color={brick.color} />
    {selected && <mesh position={[0, brick.size[1] / 2 + 0.13, 0]}>
...
function Scene({ bricks, selectedId, ghost, ghostValid, onSelect, onPointer, onCaptureReady, onPlace, viewMode, gridVisible }: {
  ...
  return <Canvas shadows camera={{ position: [8, 7, 9], fov: 45 }} gl={{ preserveDrawingBuffer: true }}>
    ...
    {ghost && <GhostBrick position={ghost.position} size={ghost.size} footprint={ghost.footprint} shape={ghost.shape} color={ghost.color} valid={ghostValid} />}
