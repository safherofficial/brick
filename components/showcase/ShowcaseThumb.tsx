...
function footprintOf(
  ...
  return [
    Math.max(1, Math.round(size[0] / STUD)),
    Math.max(1, Math.round(size[2] / STUD))
  ];
}
...
export default function ShowcaseThumb({
  creation,
  interactive = false
}: {
  creation: ShowcaseCreation;
  interactive?: boolean;
}) {
  return (
    <Canvas
